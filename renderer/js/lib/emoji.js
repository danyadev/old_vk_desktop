...
var j = (e, t) => {
  for(let r = 0; e.length > r; r++) if(e[r] == t) return r;

  return -1;
}

var inArray = (e, t) => j(t, e) != -1;
...
var getEmojiHTML = (code, symbol, enabled, is_tab) => {
  if(code.match(/2640|2642|200D|200C/g)) {
    ...
    if(!inArray(code, emojiWithJoiners)) {
      ...
      let s = typeof symbol == 'string' ? symbol : symbol[0];
  ...
  if(cssEmoji[code] != undefined) {
    ...
    return `<img src="https://vk.com/images/blank.gif" emoji="${code}" alt="${symbol}" class="emoji _emoji" style="background-position: 0px ${num}px;">`;
  } else {
    if(!imgEmoji[code] && symbol && !enabled) {
      return symbol;
    } else if(is_tab) {
      let code_lower_case = code.toLowerCase(),
          spriteId = parseInt(code_lower_case, 16) % EMOJI_SPRITES_NUM;

      return `<i class="emoji emoji_sprite_${spriteId} emoji_${code_lower_case}" emoji="${code}" alt="${symbol}"></i>`;
    } else {
...
var replace = symbolstr => {
  ...
  for(let i in codes) {
    ...
    if(buffer) {
      if(isEmoji(altBuffer)) {
        ...
        out += nmab + getEmojiHTML(bmab, tmab, true);
      } else out += altBuffer;
    }
  ...
  if(buffer) {
    if(isEmoji(altBuffer)) {
      out += getEmojiHTML(buffer, altBuffer, true);
    } else out += altBuffer;
  }
